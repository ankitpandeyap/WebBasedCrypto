...
public class JWTRefreshFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        // It no longer performs implicit refreshes for other paths based on expired access tokens.
        if (!isRefreshRequest) {
        ...
        try {
            ...
            if (refreshToken == null) {
                logger.warn("Refresh token is null or not present in cookies for explicit refresh request. Throwing BadCredentialsException.");
            ...
            logger.debug("Refresh token extracted from cookie (first 10 chars): {}", refreshToken.substring(0, Math.min(refreshToken.length(), 10)) + "...");

            if (tokenService.isBlacklisted(refreshToken)) {
                logger.warn("Refresh token is blacklisted: (first 10 chars) {}. Throwing JWTBlackListedTokenException.", refreshToken.substring(0, Math.min(refreshToken.length(), 10)) + "...");
            ...
            if (!userDetails.isEnabled()) {
                logger.warn("Refresh token owner {} profile is not enabled. Throwing exception.", userDetails.getUsername());
            ...
            // It does NOT call filterChain.doFilter(request, response) to pass the request further.
            return;
            // *** HIGHLIGHTED CHANGE END ***

        } catch (ExpiredJwtException e) {
            logger.warn("Explicit refresh token expired for user: {}. Message: {}", e.getClaims().getSubject(), e.getMessage());
            ...
            logger.warn("Explicit refresh token processing failed: Username from refresh token not found. Message: {}", e.getMessage());
            ...
            logger.error("Explicit refresh token processing failed due to unexpected error for request URI {}: {}", request.getRequestURI(), e.getMessage(), e);
